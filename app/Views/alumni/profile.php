...
<div x-data="alumniProfile()" x-init="initData()">
    ...
    return {
        ...
        async uploadCroppedPhoto() {
            ...
            this.cropperInstance.getCroppedCanvas({
                ...
                try {
                    const res = await fetch('<?= base_url('api/alumni/profile/upload-photo') ?>', {
                        method: 'POST',
                        body: formData
                    });
                    const data = await res.json();
        ...
        async saveGeneral() {
            ...
            try {
                const res = await fetch('<?= base_url('api/alumni/profile/general') ?>', {
                    method: 'POST',
        ...
        async saveRelation() {
            this.loading = true;
            const endpoint = `<?= base_url('api/alumni/profile/') ?>${this.activeRelationType}`;
        ...
        async deleteRecord(type, id) {
            ...
            this.loading = true;
            try {
                const res = await fetch(`<?= base_url('api/alumni/profile/') ?>${type}/${id}`, { method: 'DELETE' });
